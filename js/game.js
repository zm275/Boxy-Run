  ...
  function createRowOfTrees(position, probability, minScale, maxScale) {
    for (var lane = -1; lane < 2; lane++) {
      ...
      if (randomNumber < probability) {
        ...
        var tree = new Tree(lane * 800, -400, position, scale, "ancestor.jpg");
        objects.push(tree);
        scene.add(tree.mesh);
...
function createCylinder(
  ...
  height,
  radialSegments,
  imagePath, // New parameter for the image texture
  x,
  y,
  z,
  rotationY
  ...
  var texture = new THREE.TextureLoader().load("ancestor.jpg");
